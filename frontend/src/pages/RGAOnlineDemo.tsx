...
interface ResetMsg  { type: 'rga_reset' }
type RGAMsg = InitMsg | InsertMsg | RemoveMsg | ResetMsg
...
export default function RGAOnline() {
  ...
  const desiredCursor = useRef<number | null>(null)

  useLayoutEffect(() => {
    const pos = desiredCursor.current
    if (pos !== null && textareaRef.current) {
      textareaRef.current.selectionStart = pos
      textareaRef.current.selectionEnd = pos
      desiredCursor.current = null
    }
  })
  ...
  useEffect(() => {
    loadModule()
      .then(m => { moduleRef.current = m; setWasmReady(true) })
  ...
  const onMessage = useCallback((raw: unknown) => {
    ...
    } else if (msg.type === 'rga_reset') {
      ref.current?.delete()
      ref.current = new M.RGA(nodeIdRef.current) as RGAHandle
      setText('')
      return
    }
    setText(rga.text())
  ...
  const connection = useConnection('/rga', onMessage, wasmReady)

  function reset() {
    const M = moduleRef.current
    if (!M || nodeIdRef.current < 0) return
    ref.current?.delete()
    ref.current = new M.RGA(nodeIdRef.current) as RGAHandle
    setText('')
    connection.send({ type: 'rga_reset' })
  }
