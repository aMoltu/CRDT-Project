...
interface ResetMsg     { type: 'gcounter_reset' }
type GCMsg = InitMsg | IncrementMsg | ResetMsg
...
export default function GCounterOnline() {
  ...
  const onMessage = useCallback((raw: unknown) => {
    ...
    } else if (msg.type === 'gcounter_reset') {
      setSlots(prev => {
        const next = new Map(prev)
        for (const k of next.keys()) next.set(k, 0)
        return next
      })
    }
  }, [])
  ...
  function reset() {
    ...
    connection.send({ type: 'gcounter_reset' })
  }
